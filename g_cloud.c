...
qboolean CA_VerifyServerAuth(void)
{
    ...
    size_t dec_len;
    size_t enc_len;
    byte response[RSA_LEN]; // a ciphertext response to an auth challenge
    byte response_plain[RSA_LEN]; // the cleartext response to an auth challenge
    byte challenge_hash[SHA256_DIGEST_LENGTH]; // the hash we calculate
    byte response_hash[SHA256_DIGEST_LENGTH]; // the one the server sends us
    byte sv_challenge[CHALLENGE_LEN];
    uint8_t offset = 0; // used to find the parts of server's auth response

    c = &cloud.connection;

    q2a_memset(response, 0, RSA_LEN);
    len = CA_ReadShort();
    CA_ReadData(response, len);

    dec_len = G_PrivateDecrypt(response_plain, response, RSA_LEN);
    if (dec_len == 0) {
    ...
    if (cloud_encryption) {
        ...
        q2a_memcpy(c->iv, response_plain + offset, sizeof(c->iv));
    ...
    if (q2a_memcmp(challenge_hash, response_hash, SHA256_DIGEST_LENGTH) == 0) {
