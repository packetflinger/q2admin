...
qboolean CA_VerifyServerAuth(void)
{
    ...
    q2a_memcpy(sv_hash, sv_auth_plain + blob_offset, SHA256_DIGEST_LENGTH);
    blob_offset += SHA256_DIGEST_LENGTH;

    // random nonce generated by server (we hash it and send back)
    q2a_memcpy(sv_challenge, sv_auth_plain + blob_offset, CHALLENGE_LEN);
    ...
    if (q2a_memcmp(cl_hash, sv_hash, SHA256_DIGEST_LENGTH) == 0) {
        servertrusted = qtrue;
        CA_printf("server authenticated\n");
    } else {
