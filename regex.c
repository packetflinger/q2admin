...
int size2;
{
	...
	if (where == NULL)
		...
		if (FIRST_STRING_P(where)) {
			for (this_char = where - string1;
			     this_char < size1; this_char++)
			...
			case '(':
				...
				if (regnum <= MAX_REGNUM) {
					COMPILE_STACK_TOP.
					    inner_group_offset =
					    b - bufp->buffer + 2;
					BUF_PUSH_3(start_memory, regnum,
						   0);
				}
...
unsigned char *b;
{
	...
	for (this_char = range_start; this_char <= (unsigned) range_end;
	     this_char++) {
...
common_op_match_null_string_p(), group_match_null_string_p();
...
int stop;
{
	...
	if (bufp->re_nsub) {
		...
		regstart = regend = old_regstart = old_regend =
		    best_regstart = best_regend = reg_dummy = NULL;
				...
				if (mcnt < 0
					...
					if (EVER_MATCHED_SOMETHING
						...
						for (r = *p;
							...
							/* xx why this test?  */
							if ((int) old_regend[r] >= (int) regstart[r]) regend[r] = old_regend[r];
						}
					}
					p1++;
					EXTRACT_NUMBER_AND_INCR(mcnt, p1);
...
const char *s;
{
	...
	if (!re_comp_buf.buffer) {
		...
		re_comp_buf.fastmap =
		    (char *) q2admin_malloc(1 << BYTEWIDTH);
	...
	ret =
	    regex_compile(s, q2a_strlen(s), re_syntax_options,
			  &re_comp_buf);
